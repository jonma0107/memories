...
export default class Mesh {
  constructor(stage) {
    ...
    this.UnrealBloomPass = new UnrealBloomPass(new Vector2(this.stage.width, this.stage.height), 0.0, 1.4, 0.0);
  ...
  init() {
    for (let i = 0; i < 3; i++) {
      this.promiseList.push(
        new Promise((resolve) => {
          ...
          this.img[i].src = `./assets/images/memorie0${(i + 1.0)}.png`;

          this.img[i].addEventListener('load', () => {
            this.imgData[i] = new ImagePixelFilter(this.img[i], this.img[i].width, this.img[i].height, 3);
            resolve();
          });
        })
      )
    ...
    Promise.all(this.promiseList).then(() => this._setMesh(), this._setGui());
  }
